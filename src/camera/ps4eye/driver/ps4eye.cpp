...
#define UVC_STREAM_FID	(1 << 0)
	...
	*/
	...
	{
		int i;
		debug_verbose("BEGIN MULTIWRITE USB brequest a7\n");
		...
		for (i = 8; i<16; i++)
		{
			debug_verbose("0x%02x ", buffer_out[i]);
		}
		...
		sleep_millisecond(1);
		delete[] buffer_out;
		debug_verbose("END MULTIWRITE USB\n");
	}
	...
	*/
	void PS4EYECam::multi_register_write(const uint16_t data[][2], uint8_t len, uint8_t subaddr)
	{
		...
		int i;
		debug_verbose("BEGIN MULTIWRITE USB brequest a4\n");
		/* int size= ARRAY_SIZE(data);
		debug("size es %d\n",size);*/
		uint8_t *buffer_out = new unsigned char[len * 4 + 16]();
		switch (subaddr) {
	...
	*/
	void PS4EYECam::dump_registers(uint16_t start, uint16_t finish, uint8_t subaddr)
	{
		...
		uint8_t val = 0;
		debug_verbose("READ register BEGIN from 0x%04x - 0x%04x\n ", start, finish);
		while (start <= finish)
		{
			val = register_read(start, subaddr);
			if (val >= 0)
			{
				debug_verbose("READ register 0x%04x: 0x%02x\n ", start, val);
			}
			else
			{
				debug_verbose("READ register 0x%04x: READ ERROR ", start);
			}
			//   sleep(1);
			start++;
		}
		debug_verbose("READ FROM SUBADDR 0x%02x END \n", subaddr);
	}
	/*
	...
	*/
	void PS4EYECam::multi_register_read(const uint16_t(*data), uint8_t len, uint8_t subaddr)
	{
		...
		for (i = 0; i<len; i++)
		{
			...
			{
				debug_verbose("READ register 0x%04x: READ ERROR ", data[i]);
			}
			// sleep(1);
		}
	}
	...
	*/
	...
	{
		debug_verbose("reg: 0x%02x, val: 0x%02x", reg, val);
	...
	*/
	uint8_t PS4EYECam::register_read(uint16_t reg, uint8_t subaddr)
	{
		...
		debug_verbose("read  register 0x%04x  value 0x%02x \n",reg,buffer_in[18]);
		return buffer_in[18];
	...
	*/
	...
	void PS4EYECam::dump_device()
	{
		...
		if (res)
		{
			debug("error getting device descriptor: %s\n", libusb_error_name(res));
			return;
		}
	...
	bool PS4EYECam::open_usb()
	{
		...
		if (res != 0) {
			debug("set configuration error: %s\n", libusb_error_name(res));
			return false;
		}
		...
		if (res != 0) {
			debug("device claim interface error: %s\n", libusb_error_name(res));
			return false;
		}
		...
		if (res != 0) {
			debug("set alt setting to interface error: %s\n", libusb_error_name(res));
			return false;
		}
