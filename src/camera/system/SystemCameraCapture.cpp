...
bool SystemCameraCapture::Init(const int deviceIndex)
{
	...
	if (success) {
		...
		cap->set(CV_CAP_PROP_FRAME_WIDTH, ConfigHandler::GetValue("CAMERA.GENERIC.PREFERRED_RES_X", 640).asInt()); // 480
		cap->set(CV_CAP_PROP_FRAME_HEIGHT, ConfigHandler::GetValue("CAMERA.GENERIC.PREFERRED_RES_Y", 480).asInt()); // 640
		return true;
	}
	return false;
}
